...
import AddUser from '../../shared/AdminCreateButton/AddUser';
...
const { user }  = useAuth();
  ...
  const [dealers,          setDealers]          = useState([]);   // admin's dealers (for form + lookup)
  ...
  const colLabels = {
    sn:'SN',dealerName:'Dealer_Name', username:'User_name',contact:'Full_Name',
    vehicles:'No. of Vehicles', phone:'Phone No.',
  ...
  // ── Helpers ────────────────────────────────────────────────────────────────
  const getDealerName = useCallback((dealerId) => {
    if (!dealerId) return '—';
    const d = dealers.find(x => Number(x.user_id) === Number(dealerId) || x._id === dealerId);
    return d ? (d.fullName || d.name || d.username || '—') : '—';
  }, [dealers]);
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      // Dealers for this admin (backend filters by adminId for admin role)
      const dealersRes = await getUsersApi({ limit: 2000 });
      const dealerData = (dealersRes.data.users || []).filter(u => u.role === 'dealer');
      setDealers(dealerData);

      // Users under this admin — use filterRole=user so backend returns
      // all users where adminId = admin.user_id
      const usersRes  = await getUsersApi({ limit: 5000, filterRole: 'user' });
      const userData  = (usersRes.data.users || []).filter(u => u.role === 'user');
      setUsers(userData);

    } catch (err) {
      console.error('[UserManagement] fetchAll error:', err);
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      if (editingUser) {
        ...
        ['superAdminId','adminId','dealerId','parentId','createdBy','role']
  ...
  const toggleSuspension = async (u) => {
    ...
    try {
      ...
      toast.success(willSuspend ? 'User suspended successfully' : 'User unsuspended successfully');
      fetchAll();
    } catch (err) {
  ...
  const clearFilters = () => {
    setStatusFilter('ALL'); setSelectedMonth('');
    setSelectedYear('');    setSearch('');    setPage(1);
  };
  ...
  const filtered = users
    ...
    .filter(u => {
      ...
      return ['username','fullName','name','email','phone','company','address','ownerName']
        .some(k => (u[k] || '').toLowerCase().includes(q));
    })
    .sort((a, b) => {
      const map = {
        username:   'username',  status:     'status',
        vehicles:   'maxVehicles', dealerName: 'dealerId',
...
const tdStyle = {
 ...
 <div style={{ padding:'0', background:'#f8fafc', height:'100%', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:15, flexWrap:'wrap', flex:1 }}>
     ...
     <div style={{ borderTop:'none', overflowX:'auto', background:'#fff', width:'100%', flex:1 }}>
          ...
          <thead>
            <tr>
              ...
              {visibleCols.dealerName   && <th style={thStyle} onClick={() => handleSort('dealerName')}>Dealer_Name <SortIcon col="dealerName" sortCol={sortCol} sortDir={sortDir} /></th>}
            ...
            {loading ? (
              ...
              paginated.map((u, idx) => {
                ...
                return (
                  ...
                  <tr key={u._id}
                    style={{
                      background: isSus ? '#fff5f5' : (idx % 2 === 0 ? '#fff' : '#fafbfc'),
                      transition: 'background 0.1s', cursor: 'pointer'
                    }}
                    onMouseEnter={e => e.currentTarget.style.background = '#f0f4ff'}
                    onMouseLeave={e => e.currentTarget.style.background = isSus ? '#fff5f5' : (idx % 2 === 0 ? '#fff' : '#fafbfc')}
                    onClick={() => handleEditClick(u)}
                  >
                    {visibleCols.sn && <td style={tdStyle}>{(page - 1) * rowsPerPage + idx + 1}</td>}


                    {visibleCols.dealerName && (
                      <td style={tdStyle}>{getDealerName(u.dealerId)}</td>
                    )}
                    ...
                    {visibleCols.autoExp     && <td style={tdStyle}>{u.autoExpiry || u.autoExp || '—'}</td>}
                    {visibleCols.subStart    && <td style={tdStyle}>{u.subscriptionStartDate  ? new Date(u.subscriptionStartDate).toLocaleDateString('en-IN')  : '—'}</td>}
                    {visibleCols.subDue      && <td style={tdStyle}>{u.subscriptionDueDate    ? new Date(u.subscriptionDueDate).toLocaleDateString('en-IN')    : '—'}</td>}
                    {visibleCols.subExtended && <td style={tdStyle}>{u.subscriptionExtendDate ? new Date(u.subscriptionExtendDate).toLocaleDateString('en-IN') : '—'}</td>}
                    ...
                    {visibleCols.delete && (
                      <td style={{ ...tdStyle, textAlign:'center' }} onClick={e => e.stopPropagation()}>
                        {isInactive ? (
                          ...
                          <span title="Cannot delete active user. Please make the user inactive first."
