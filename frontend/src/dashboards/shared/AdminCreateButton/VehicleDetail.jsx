...
// ── Disabled field style (grayed out, lock icon hint) ────────────────────────
const disabledStyle = {
  background: "#f1f5f9",
  color: "#94a3b8",
  cursor: "not-allowed",
...
const Spin = ({ size = 14, color = ACC_DEFAULT }) => (
  <span style={{
    ...
    animation: "adSpin .7s linear infinite", flexShrink: 0,
  }} />
);

// ── Field wrapper ─────────────────────────────────────────────────────────────
const Fld = ({ label, req, opt, children, disabled }) => (
...
// ── Count filled optional fields (12 fields) ─────────────────────────────────
const countFill = v => {
  const fields = [
...
// MAIN FORM COMPONENT
// ═════════════════════════════════════════════════════════════════════════════
export default function VehicleAdditionalDetailsForm({
  isOpen,
  onClose,
  onSaved,
  currentUser,
  allVehicles = [],
  initialVehicle = null,   // ← passed from table edit row click
}) {
  const theme = useTheme();
  ACC = theme?.activeColor || ACC_DEFAULT;

  // ── Edit mode: true when opened by clicking a row ────────────────────────
  // When initialVehicle exists → edit mode → lock Dealer / User / Vehicle selects
  ...
  // ── Populate form from vehicle object (12 fields) ─────────────────────────
  ...
  useEffect(() => {
    if (!isOpen) {
      setSelDealer(""); setSelUser(""); setSelVehicle(null);
  ...
  useEffect(() => {
    ...
    fetchUsers(selDealer)
      ...
      .catch(err => console.error("[Form fetchUsers]", err))
      .finally(() => setLdUsers(false));
  }, [selDealer]);

  // ── User → filter vehicles ────────────────────────────────────────────────
  useEffect(() => {
      ...
      >
        ...
        >
          ...
          <div style={{ overflowY: "auto", padding: "20px 24px", flex: 1 }}>
            ...
            {/* ── TOP 3 ROLE FIELDS — disabled in edit mode ──────────────── */}
            <div style={{
              display: "grid", gridTemplateColumns: "repeat(3, 1fr)",
              gap: "14px 18px", marginBottom: 24,
            }}>

              {/* ── Select Dealer ── DISABLED in edit mode ─────────────── */}
              <Fld label="Select Dealer" req disabled={isEditMode}>
                <div style={{ position: "relative" }}>
                  <select
                    style={{
                      ...sel,
                      ...(isEditMode ? disabledStyle : {
                        borderColor: selDealer ? ACC : "#d0d7de",
                      }),
                    }}
                    value={selDealer}
                    onChange={e => {
                    ...
                    disabled={isEditMode || ldDealers}
                  >
                    <option value="">
                      {ldDealers ? "Loading dealers…" : "— Select Dealer —"}
                    </option>
                    {dealers.map(d => (
                ...
                <div style={{ position: "relative" }}>
                  <select
                    style={{
                      ...sel,
                      ...(isEditMode ? disabledStyle : {
                        borderColor: selUser ? ACC : selDealer ? "#f59e0b" : "#d0d7de",
                        opacity: !selDealer ? 0.6 : 1,
                      }),
                    }}
                    value={selUser}
                    onChange={e => {
                    ...
                    onFocus={isEditMode ? undefined : onFocus}
                    onBlur={isEditMode ? undefined : onBlur}
                    disabled={isEditMode || !selDealer || ldUsers}
                  >
                    <option value="">
                      {ldUsers ? "Loading users…"
              ...
              <Fld label="Select Vehicle" req disabled={isEditMode}>
                ...
                >
                  ...
                  {/* In edit mode show the pre-selected vehicle */}
            ...
            {/* ── VEHICLE DETAIL FIELDS — always fully editable ──────────── */}
            ...
            <div style={{
              ...
              <Fld label="Vehicle Brand" opt>
                <input
                  style={inp} value={f.vehicle_brand}
              ...
              <Fld label="Vehicle Model" opt>
                <input
                  style={inp} value={f.vehicle_model}
              ...
              <Fld label="Vehicle Body" opt>
                <select
                  style={sel} value={f.vehicle_body}
              ...
              <Fld label="Capacity" opt>
                <input
                  style={inp} type="number" min={0} value={f.capacity}
                  placeholder="0"
              ...
              <Fld label="Odometer (km)" opt>
                <input
                  style={inp} type="number" min={0} value={f.odometer}
              ...
              <Fld label="Duration Odometer (km)" opt>
                <input
                  style={inp} type="number" min={0} value={f.duration_odometer}
              ...
              <Fld label="Manufacture Date" opt>
                <input
                  style={inp} type="date" value={f.manufacture_date}
              ...
              <Fld label="Purchase Date" opt>
                <input
                  style={inp} type="date" value={f.purchase_date}
