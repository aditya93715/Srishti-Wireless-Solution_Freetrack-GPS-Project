...
import { fetchDealers } from '../../../api/vehicleApi';
      ...
      {loadingData ? (
        ...
        <>
          <div ref={triggerRef} style={triggerStyle} onClick={toggle}>
            <span style={{ fontSize: 13, color: value ? '#1a1f2e' : '#9ca3af', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
// ─────────────────────────────────────────────────────────────────────────────
// LIVE SELECT — custom dropdown, always opens downward
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// SENSOR FORM — matches the controller schema exactly
// ─────────────────────────────────────────────────────────────────────────────
const SENSOR_TYPES = [
...
// SENSOR TAB
// ─────────────────────────────────────────────────────────────────────────────
const SensorTab = ({ deviceId, existingSensors, onSensorsChange, ACCENT, HEADER_BG, isNewDevice = false }) => {
  ...
  useEffect(() => {
    ...
    setSensorForm({ ...EMPTY_SENSOR }); setCalibRows([]);
    setSensorError(''); setSearch('');
  ...
  const openAddForm = () => {
    setSensorForm({ ...EMPTY_SENSOR, calibrationRows: [] }); setCalibRows([]);
    setEditingIdx(null); setSensorError(''); setShowForm(true);
  };
  ...
  const openEditForm = (idx, e) => {
    ...
    setEditingIdx(idx); setSensorError(''); setShowForm(true);
  };

  const closeForm = () => {
    setShowForm(false); setEditingIdx(null);
    setSensorError(''); setSensorForm({ ...EMPTY_SENSOR, calibrationRows: [] }); setCalibRows([]);
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#f8fafc', border: '1px solid #e2e8f0', padding: '0 10px', height: 34, borderRadius: 3, minWidth: 220 }}>
        ...
        <button type="button" onClick={openAddForm}
          style={{ height: 34, padding: '0 18px', background: HEADER_BG, color: '#fff', border: 'none', borderRadius: 3, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <thead>
            <tr style={{ background: HEADER_BG }}>
              {['#', 'Sensor Type', 'Connected To', 'Name', 'Type', 'Inverse', 'RS232', 'Dashboard', 'Delete'].map(h => (
                <th key={h} style={{ padding: '10px 14px', textAlign: h === '#' ? 'center' : 'left', fontSize: 12, fontWeight: 600, color: '#fff', whiteSpace: 'nowrap', borderRight: '1px solid rgba(255,255,255,0.08)' }}>
                  {h}
                </th>
              ))}
            </tr>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={9} style={{ padding: '32px 0', textAlign: 'center', color: '#9ca3af', fontSize: 13 }}>
                  {sensors.length === 0 ? 'No sensors added yet. Click "+ Add" to add the first sensor.' : 'No sensors match your search.'}
                </td>
              </tr>
            ) : filtered.map((s, i) => {
              ...
              return (
                <tr key={s.sensor_id || i}
                  ...
                  <td style={{ padding: '9px 14px', textAlign: 'center', borderBottom: '1px solid #f1f5f9', color: '#6b7280', fontSize: 12 }}>{s.sensor_id}</td>
                  ...
                  <td style={{ padding: '9px 14px', borderBottom: '1px solid #f1f5f9' }}>
                    {s.type ? <span style={{ padding: '2px 8px', background: '#ede9fe', color: '#5b21b6', borderRadius: 3, fontSize: 11, fontWeight: 500 }}>{s.type}</span> : '—'}
                  </td>
                  {['inverse','rs232','showOnDashboard'].map(flag => (
                    <td key={flag} style={{ padding: '9px 14px', textAlign: 'center', borderBottom: '1px solid #f1f5f9' }}>
                      <span style={{ display: 'inline-block', width: 20, height: 20, lineHeight: '20px', textAlign: 'center', borderRadius: 3, fontSize: 11, fontWeight: 700, background: s[flag] ? '#dbeafe' : '#f1f5f9', color: s[flag] ? '#1976d2' : '#94a3b8' }}>{s[flag] ? 'Y' : 'N'}</span>
                    </td>
                  ))}
                  <td style={{ padding: '9px 14px', textAlign: 'center', borderBottom: '1px solid #f1f5f9' }} onClick={e => handleSensorDelete(realIdx, e)}>
                    <button type="button" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: '#ef4444', borderRadius: 3, display: 'inline-flex' }}
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}>
          <div style={{ width: '92vw', maxWidth: 900, background: '#fff', borderRadius: 4, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', height: 48, background: HEADER_BG, flexShrink: 0 }}>
              ...
              <button type="button" onClick={closeForm}
                style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 24, lineHeight: 1, padding: 0 }}>×</button>
            </div>

            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
              <div style={{ flex: 1, padding: '24px 28px', overflowY: 'auto', borderRight: '1px solid #e2e8f0' }}>
                {sensorError && (
                  <div style={{ padding: '8px 12px', marginBottom: 14, background: '#fff5f5', border: '1px solid #fca5a5', borderLeft: '3px solid #ef4444', borderRadius: 3, fontSize: 12, color: '#dc2626' }}>
                    {sensorError}
                  </div>
                )}
              ...
              {/* Calibration Values Panel */}
              <div style={{ width: 360, padding: '24px 20px', background: '#f8fafc', overflowY: 'auto', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
                ...
                {calibRows.length > 0 ? (
                  ...
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9ca3af', fontSize: 12, flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, padding: '12px 24px', borderTop: '1px solid #e2e8f0', background: '#f8fafc', flexShrink: 0 }}>
              <button type="button" onClick={closeForm}
              ...
              <button type="button" onClick={handleSensorSave} disabled={saving}
                style={{ height: 36, padding: '0 32px', background: saving ? '#a78bfa' : HEADER_BG, color: '#fff', border: 'none', borderRadius: 3, fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                {saving
...
const AddDevices = ({
  ...
  // Build form state from a Device_Master document (edit mode).
  ...
  useEffect(() => {
    ...
    if (editDevice) {
      ...
      setBufferedSensors(editDevice.sensors || []);
    } else {
      setForm({ ...EMPTY });
      setBufferedSensors([]);
    }
    ...
    Promise.allSettled([
      ...
      if (dealerRes.status === 'fulfilled') {
        const rawList = Array.isArray(dealerRes.value) ? dealerRes.value : [];
        ...
        if (list.length === 0) {
          setClientError('No dealer accounts found under your account.');
        }
      } else {
        const msg = dealerRes.reason?.response?.data?.message || dealerRes.reason?.message || '';
        setClientError(`Could not load clients: ${msg}`);
      }

    }).finally(() => setLoadingData(false));

  }, [open, editDevice, buildForm, currentUser, getCoinsFromUser]);
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.client)            return setError('Client is required');
    ...
    if (!editDevice) {
      ...
      // ✅ Only show error when coins <= 0 (no positive message)
 ...
 const inp = {
  ...
  const sFocus = e => { e.target.style.borderColor = ACCENT; e.target.style.boxShadow = `0 0 0 2px ${ACCENT}30`; };
  const sBlur  = e => { e.target.style.borderColor = '#d0d7de'; e.target.style.boxShadow = 'none'; };
  ...
  return (
      ...
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px 0', overflowY: 'auto' }}>
      <div style={{ width: '96vw', maxWidth: 1420, background: '#f5f7fa', borderRadius: 4, display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', overflow: 'hidden', maxHeight: '90vh', margin: 'auto' }}>
        ...
        <div style={{ display: 'flex', background: '#f0f2f5', borderBottom: '1px solid #dde1e7', flexShrink: 0 }}>
          {[{ id: 'device', label: 'Add Device' }, { id: 'sensor', label: 'Add Sensor Details' }].map(t => (
            <button key={t.id} type="button" onClick={() => setTab(t.id)}
              style={{ padding: '12px 28px', fontSize: 13, fontWeight: tab === t.id ? 600 : 400, border: 'none', borderBottom: tab === t.id ? `3px solid ${ACCENT}` : '3px solid transparent', background: tab === t.id ? '#fff' : 'transparent', color: tab === t.id ? ACCENT : '#6b7280', cursor: 'pointer' }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ padding: '24px 28px 20px', background: '#fff', flex: 1, overflowY: 'auto' }}>
            ...
            {protoError && (
              <div style={{ padding: '8px 12px', marginBottom: 10, background: '#fffbeb', border: '1px solid #fcd34d', borderLeft: '3px solid #f59e0b', borderRadius: 3, fontSize: 12, color: '#92400e' }}>⚠ {protoError}</div>
            )}
            {clientError && (
              <div style={{ padding: '8px 12px', marginBottom: 10, background: '#fff5f5', border: '1px solid #fca5a5', borderLeft: '3px solid #ef4444', borderRadius: 3, fontSize: 12, color: '#dc2626' }}>⚠ {clientError}</div>
            )}
            {error && (
              <div style={{ padding: '8px 12px', marginBottom: 12, background: '#fff5f5', border: '1px solid #fca5a5', borderLeft: '3px solid #ef4444', borderRadius: 3, fontSize: 12, color: '#dc2626' }}>{error}</div>
            )}


            {tab === 'device' && (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '14px 20px', marginBottom: 18 }}>

                  {/* ── IMEI — fully editable in create AND edit mode ── */}
                  <div>
                    <Label text="IMEI" req />
                    <input
                      style={inp}
                  ...
                  <div>
                    ...
                    <input style={inp} value={form.sim_card} onChange={e => set('sim_card', e.target.value)}
                  ...
                  <div>
                    ...
                    <input style={inp} value={form.secondarySimCard} onChange={e => set('secondarySimCard', e.target.value)}
                      placeholder="SIM Number" onFocus={sFocus} onBlur={sBlur} />
                    <div style={{ fontSize: 10, color: '#9ca3af', marginTop: 3 }}>Only contains 24 digit</div>
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '14px 20px', marginBottom: 24 }}>
                  ...
                  {/* ── CLIENT (DEALER) DROPDOWN ──────────────────────────────
                      Lists dealers under this admin (from getClientOptions).
                      `value` = dealer's username (matches Device_Master.client).
                      On edit, pre-selects the dealer stored in device.client.
                      On save, the selected username is sent as `client` to backend.
                  ──────────────────────────────────────────────────────────── */}
                  <ClientDropdown
                    clients={clients}
                    value={form.client}
                    onChange={(username, clientObj) => {
                      ...
                      setSelectedClientStatus(clientObj ? { status: clientObj.status, active: clientObj.active } : null);
                    }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    {selectedClientStatus ? (
                      <span style={{
                        ...
                        <span style={{ width: 7, height: 7, borderRadius: '50%', background: selectedClientStatus.active ? '#4caf50' : '#ef4444', display: 'inline-block' }} />
            ...
            {tab === 'sensor' && (
              <SensorTab
                ...
                existingSensors={editDevice?.sensors || bufferedSensors}
                onSensorsChange={(updatedSensors) => {
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, padding: '12px 24px', borderTop: '1px solid #e2e8f0', background: '#f8fafc', flexShrink: 0 }}>
            ...
            {tab === 'device' && (
              <button type="submit" disabled={loading || coinLoading}
                style={{ height: 36, padding: '0 32px', background: (loading || coinLoading) ? '#a78bfa' : ACCENT, color: '#fff', border: 'none', borderRadius: 3, fontSize: 13, fontWeight: 600, cursor: (loading || coinLoading) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                {(loading || coinLoading)
