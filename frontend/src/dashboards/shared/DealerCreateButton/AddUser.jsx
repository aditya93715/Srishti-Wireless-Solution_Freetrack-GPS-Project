...
const AddUser = ({
  ...
  const dealerDisplayName = currentUser?.fullName || currentUser?.name || currentUser?.username || '';
  ...
  const buildEditForm = (u) => ({
    ...
    // ✅ allocatedCoins COMPLETELY REMOVED
  });
  ...
  const handleSubmit = e => {
    ...
    // ✅ allocatedCoins REMOVED - set to 0
    payload.allocatedCoins = 0;

    if (!isEdit) {
      payload.superAdminId = currentUser?.superAdminId ?? null;
      payload.adminId      = currentUser?.adminId      ?? null;
      payload.dealerId     = currentUser?.user_id;
      payload.parentId     = currentUser?.user_id;
      payload.createdBy    = currentUser?.user_id;
      payload.ownerId      = currentUser?.user_id;
      payload.ownerName    = dealerDisplayName;
    }
  ...
  return (
    ...
    >
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          <form onSubmit={handleSubmit} autoComplete="off">
            <div style={{ padding: '18px 24px 14px', background: '#fff' }}>
              ...
              {activeTab === 1 && (
                <>
                  {/* Row 1: Username / Password / Confirm Password / Full Name */}
                  <div style={GRID4}>
                    <div>
                      <L text="Username" req />
                  ...
                  {/* Row 2: Company / Email / Phone / Address */}
                  <div style={GRID4}>
                  ...
                  {/* Row 3: Language / Max Vehicle / Timezone / Auto Expiry */}
                  <div style={GRID4}>
                    <div>
                      <L text="Language" req />
                  ...
                  {/* Row 5: Checkboxes only (Coin field removed) */}
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px 18px', marginBottom: 14 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, gridColumn: 'span 4', paddingTop: 18 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, padding: '12px 24px', borderTop: '1px solid #e5e7eb', background: '#f9fafb' }}>
              ...
              <button type="submit" disabled={loading}
                ...
                {loading
                  ? <><div style={{ width: 13, height: 13, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'dealerUserSpin .7s linear infinite' }} />Saving…</>
                  : isEdit ? 'UPDATE' : 'SAVE'
                }
              </button>
            </div>
      ...
      <style>{`@keyframes dealerUserSpin { to { transform: rotate(360deg); } }`}</style>
    </div>
  );
};
