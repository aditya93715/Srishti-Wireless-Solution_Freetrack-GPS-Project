...
const UserManagement = () => {
  ...
  const [visibleCols, setVisibleCols] = useState({
    sn: true,adminName: true,dealerName: true,username: true,contact: true,
    vehicles: true,  phone: true, email: true,
    company: true, timezone: true, status: true,
    autoExp: true, subStart: true, subDue: true, subExtended: true,
    language: true, suspension: true, showPassword: true,
    address: true, delete: true,
  });

  const colLabels = {
    sn: 'SN', username: 'User_name', dealerName: 'Dealer_Name',
    adminName: 'Admin_Name', contact: 'Full_Name', vehicles: 'No. of Vehicles',
    phone: 'Phone No.', email: 'Email',
    company: 'Company', timezone: 'Timezone', status: 'Status',
  ...
  const getAdminName = (adminId) => {
    if (!adminId) return '—';
    const a = admins.find(x => Number(x.user_id) === Number(adminId) || x._id === adminId);
    return a ? (a.fullName || a.name || a.username || '—') : '—';
  };
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      if (isSuperAdmin) {
        const [adminsRes, dealersRes, usersRes] = await Promise.all([
          getAdminsApi(),
          getDealersByAdminApi(),
          getUsersByAdminApi(),
        ]);
        setAdmins(adminsRes.data.users || []);
        setDealers(dealersRes.data.users || []);
  ...
  }, [user, toast, isSuperAdmin, isAdmin, isDealer]);
  ...
  const filtered = users
    ...
    .filter(u => {
      ...
      if (selectedMonth && selectedYear) return m === selectedMonth && y === selectedYear;
    ...
    .filter(u => {
      ...
      return ['username', 'fullName', 'name', 'email', 'phone', 'company', 'address', 'domain', 'ownerName']
        .some(k => (u[k] || '').toLowerCase().includes(q));
    })
    .sort((a, b) => {
      const map = {
        username: 'username', status: 'status', vehicles: 'maxVehicles',
        dealerName: 'dealerId', adminName: 'adminId',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 15, padding: '0 10px', borderLeft: '1px solid #e2e8f0', borderRight: '1px solid #e2e8f0', height: 36 }}>
            {[
              ...
              <label key={key} style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', fontSize: 13 }}>
                <input type="radio" name="saUserStatus"
      ...
      <div style={{ borderTop: 'none', overflowX: 'auto', background: '#fff', width: '100%', flex: 1 }}>
          ...
          <thead>
            <tr>
              ...
              {visibleCols.adminName    && <th style={thStyle} onClick={() => handleSort('adminName')}>Admin_Name <SortIcon col="adminName" sortCol={sortCol} sortDir={sortDir} /></th>}
            ...
            {loading ? (
              ...
              paginated.map((u, idx) => {
                ...
                >
                    ...
                    {visibleCols.adminName  && <td style={tdStyle}>{getAdminName(u.adminId)}</td>}
                    {visibleCols.dealerName && <td style={tdStyle}>{getDealerName(u.dealerId)}</td>}
                    ...
                    {/* ── Subscription fields — correct DB field names ── */}
