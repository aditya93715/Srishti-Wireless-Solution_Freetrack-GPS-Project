...
// ── Edit form style tokens (same as dealer) ───────────────────────────────────
const ainp = {
...
// ── EDIT MODAL (user role — same layout as dealer, no user/device section) ────
const VehicleEditModalUser = ({ vehicle, onClose, onSaved }) => {
...
// ── COLUMNS (no IMEI, no SIM, no user_id, no delete action) ──────────────────
const ALL_COLS = [
...
const DEFAULT_VIS = {
  sn:true,
  vehicle_no:true, vehicle_type:true,
...
// ── MAIN ──────────────────────────────────────────────────────────────────────
export default function VehiclesUser({ currentUser }) {
  ...
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      ...
      console.error("[VehiclesUser fetchVehicles]", err);
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [vehicles, search, vstatus, filterMonth, sortCol, sortDir]);
  ...
  return (
    <div style={{ padding:"0", background:"#f4f6f9", height:"100%", display:"flex", flexDirection:"column", fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", background:"#fff", borderBottom:"1px solid #e2e8f0", flexWrap:"wrap" }}>
        ...
        {/* Search */}
        <div style={{ display:"flex", alignItems:"center", gap:6, background:"#f8fafc", border:"1px solid #e2e8f0", padding:"0 10px", height:38, borderRadius:3, minWidth:260 }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5">
            <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
          </svg>
          <input placeholder="Search vehicle no., owner, brand, model…" value={search}
        ...
        {/* NOTE: No Add Vehicle button for user role */}
      </div>
